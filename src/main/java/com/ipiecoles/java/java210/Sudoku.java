...
public class Sudoku {
	...
	public void ecrireSudoku(short[][] sudoku) {
		...
		for (int i = 0; i < 9; ++i) {
			...
			for (int j = 0; j < 9; ++j) {
				...
				chaineSudoku += sudoku[i][j] == 0 ? " " : Integer.toString(sudoku[i][j]);
	...
	public boolean resoudre(int abscisse, int ordonnee, short[][] sudoku) {
		...
		// Sinon, on essaye chaque valeur dans la case en appelant la méthode
		// estAutorise
